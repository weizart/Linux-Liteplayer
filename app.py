...
# 基础目录，修改为自己的任务路径
BASE_DIR = "/mnt/jfs2/hdd2/sora/data/videos/purchase/movie_baidudisk/data/3-1080p-20241108/task_3/_movies"
...
def display_files(index, df):
    ...
    if not all_files:
        ...
        return
    # 检查视频数量
    if len(all_files) > SERIES_THRESHOLD:
        is_series = st.radio(
            "检测到当前文件夹包含多个视频文件，是否为剧集文件夹？",
            ("是", "否"),
            index=0,
            help="选择'是'将只加载第一个视频，对其标注将应用到所有文件；选择'否'将加载所有视频，可能过慢"
        )
        ...
        if is_series == "是":
            ...
            file_size_mb, duration = get_file_metadata(first_file)

            # 从 DataFrame 中获取第一个文件的设置
            row = df[df['path'] == first_file]
            if not row.empty:
                row = row.iloc[0]
            ...
            # 添加一个确认按钮
            if st.button("将当前设置应用到所有视频"):
                # 更新 DataFrame 中所有相关文件的设置
                for file_path in all_files:
                    df.loc[df['path'] == file_path, 'discard'] = '是' if discard else '否'
                    df.loc[df['path'] == file_path, 'human_subtitle'] = subtitle
                    df.loc[df['path'] == file_path, 'human_border'] = border
                    df.loc[df['path'] == file_path, 'human_watermark'] = watermark
                    df.loc[df['path'] == file_path, 'human_beginning'] = f"{h:02d}:{m:02d}:{s:02d}"
                st.success(f"已将设置应用到当前目录的所有 {len(all_files)} 个视频文件")
            
            # 显示其他文件信息
            st.write(f"目录中共有 {len(all_files)} 个视频文件:")
